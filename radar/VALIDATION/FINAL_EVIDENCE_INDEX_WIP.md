# CRT V1.10 Final Evidence Index

## Formal authority

- Formal version: `CRT V1.10`
- Integration vehicle: Pull Request `#1`
- Formal seal approval: `2026-08-05 19:09 +08:00`
- Production: `NOT_APPROVED`
- External action authority: `NONE`

## Formal seal and state

- `RELEASE/CRT_V1.10_FORMAL_SEAL_20260805.md`
- `STATUS/CURRENT_WIP_STATE.md`
- `STATUS/WORKSPACE_STATE.json`
- `RELEASE/PRE_SEAL_CHECKLIST_WIP.md`
- `RELEASE/ROLLBACK_PLAN_WIP.md`
- `RELEASE/HANDOFF_WIP.md`

## Accepted Live evidence

- `VALIDATION/LIVE_SHADOW_PASS_20260805/EVIDENCE_MANIFEST.json`
- `P1_01_POST_LIVE_REVIEW_20260805.md`
- Accepted process run ID: `17cccceb-d819-4fca-aab0-45f6dc9275f1`
- Final decision: `LIVE_SHADOW_PASS`
- Acceptance failures: `0`
- External runtime ZIP SHA256: `a30c518317621c05e22f7705682169c4e7adf28f1025a8d7d5739d92060223e7`
- Runtime manifest SHA256: `a83f25ffb1e45fa2d8b299b75b00c98160703130d564ece9974d79553a6f7ce9`

## Regression and governance evidence

- `VALIDATION/INTEGRATED_COMPILE_REPORT.txt`
- `VALIDATION/INTEGRATED_TEST_LOG.txt`
- `VALIDATION/LEGACY_V0.2_RERUN_LOG.txt`
- `VALIDATION/PR1_FILE_AUDIT_20260801.md`
- `CONFIG/SOURCE_REGISTRY_V1.2.json`
- `CONFIG/LIVE_SHADOW_POLICY_V1.json`
- `registry/RADAR_SYSTEM_REGISTRY.csv`

## Post-seal work

- `POST_SEAL_20_RUN_MATURITY_PLAN.md`
- `STATUS/POST_SEAL_20_RUN_TRACKER.csv`
- Maturity progress: `0 / 20`

## Still blocked

- Production approval
- Trading or account access
- Email, webhook or notification
- Any external transaction authority
- Applying or discarding the parallel automation stash without separate review
